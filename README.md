...
当前版本：**v1.3.3** · 应用名 `openclaw.studio` · 依赖 `nodejs_v24` · 要求 fnOS ≥ 1.1.3100
...
飞牛把主题偏好存成**枚举**而不是单词：`10` = 浅色、`20` = 深色、`30` = 跟随系统
（写在其 `localStorage` 的 `fnos-theme-mode` / `os-theme-mode` 里）。

此前我们把这个值**原样**写进页面的 `theme-mode` 属性（即 `theme-mode="30"`），
而样式规则是 `html[theme-mode="dark"]` —— 永远匹配不上，于是无论飞牛怎么设置，
启动器界面都**固定停在浅色**。因为它看起来"有在读取设置"，这个 bug 一直没被怀疑。

现在：

- 正确解析枚举（`10`/`20`/`30`），并在「跟随系统」时交给系统配色决定
- 监听 `storage` 事件，**切换主题无需刷新页面**即可生效
- 监听 `prefers-color-scheme` 变化，系统深浅色切换时同步
- 兼容直接写 `dark` / `light` 的旧写法

回归测试见 `scripts/test-theme.mjs`（9 项，含免刷新实时切换）。

> 顺带修正了应用介绍里仍残留的已移除功能说明（原文还在宣传"内置 17 个模型服务商预设"
> 与"模型面板"，那是 v1.2.0 就删掉的东西）。应用中心列表里显示的就是这段文字。
...
除 `test-theme.mjs` 外都会在出现任何控制台错误、页面异常或失败请求时以非零码退出。

> `scripts/deploy.sh` 把当前源码同步进**已安装**的应用并重启，用于免卸载迭代。
> 注意 `appcenter-cli install-fpk` **不会**升级已安装的应用（它只做全新安装），
> 升级请走应用中心界面的「手动安装」，或卸载后重装（数据目录会保留）。
...
| 版本 | 内容 |
|---|---|
| **v1.3.3** | 修复界面不跟随飞牛桌面深色模式（主题值是 10/20/30 枚举，此前被原样写入导致永远浅色）；支持免刷新实时切换 |
